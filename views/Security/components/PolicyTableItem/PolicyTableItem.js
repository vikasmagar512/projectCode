								...
								<div>
									<p className={'medium strong'}>
										{service.port
											...
											: service.type
											? `${service.protocol.toUpperCase()}/${service.type}`
											: service.protocol.toUpperCase()}
									</p>
									<p className={'small'}>{service.name}</p>
								</div>
							</div>
						)
					})
...
class PolicyTableItem extends React.PureComponent {
	constructor(props) {
		super(props)
		...
		return (
			<TableItemContainer active={this.state.showAll}>
				...
				<WrapperComponent title={'Service'} extraClass={'field__service'}>
					<ServiceField services={data.services} />
				</WrapperComponent>
				<WrapperComponent
					title={'Application'}
...
export default connect(mapStateToProps)(PolicyTableItem)
