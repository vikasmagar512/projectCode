...
import NewAddressTranslation from '../Modals/NewAddressTranslation'
import AddressTranslationTableItem from './components/AddressTranslationTableItem/AddressTranslationTableItem'
		...
		<div>
			{cards.map((card, i) => (
				<DndCard
					...
					onReorder={onReorder}
					TableItemComp={AddressTranslationTableItem}
					matches={matches}
				/>
			))}
		</div>
...
class AddressTranslation extends Component {
	state = {
			...
			<div className="content policies">
				...
				<div>
					...
					<WedgeModal
						...
						title={'Create new address translation'}
						size={'big'}
					>
...
AddressTranslation.defaultProps = {
	items: [],
	objects: []
}
...
const policiesSelector = state => {
	const ecosystem = state.ecosystems.currentEcosystem
	if (ecosystem) {
		return state.DNATs[ecosystem.uuid] ? state.DNATs[ecosystem.uuid].DNATs : []
	}
...
const mapDispatchToProps = dispatch => {
	return {
		fetchPolicies: () => dispatch(fetchDNATs()),
		fetchObjects: ecosystemUUID => dispatch(fetchObjects(ecosystemUUID)),
		createPolicy: (entity, type) => dispatch(createDNAT(entity, type)),
		createObject: (entity, type) => dispatch(createObject(entity, type)),
		updatePolicy: (entity, uuid) => dispatch(updateDNAT(entity, uuid)),
		deletePolicy: entity => dispatch(deleteDNAT(entity)),
		reorderPolicy: (policy, after) => dispatch(reorderDNAT(policy, after)),
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(MyTagControlContext(AddressTranslation))
